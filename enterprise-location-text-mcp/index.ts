...
// Type definitions for text-based location search
interface TextSearchParams {
  cityName: string;
  countryCode: string; // US, CA, MX
  brand?: string; // Default ENTERPRISE
  includeExotics?: boolean; // Default true
  dto?: boolean; // Default true
  cor?: string; // Country of residence, default US
  locale?: string; // Default en_US
  stateCode?: string; // Optional state code for US searches
}
...
class EnterpriseLocationTextServer {
  ...
  private baseUrl = "https://prd.location.enterprise.com/enterprise-sls/search/location/enterprise/web";
      ...
      try {
        switch (name) {
          case "search_locations_by_city":
            return await this.searchLocationsByCity(args as unknown as TextSearchParams);
            
          case "search_locations_by_state":
            return await this.searchLocationsByState(args as unknown as any);
            
          case "search_locations_by_airport":
            return await this.searchLocationsByAirport(args as unknown as any);
  ...
  private async makeTextSearchRequest(searchTerm: string, params: Partial<TextSearchParams>): Promise<any> {
  ...
  private async searchLocationsByCity(params: TextSearchParams) {
    try {
  ...
  private async searchLocationsByState(params: any) {
    try {
  ...
  private async searchLocationsByAirport(params: any) {
    try {
